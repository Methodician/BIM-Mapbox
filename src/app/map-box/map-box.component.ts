...
export class MapBoxComponent implements OnInit {
  ...
  constructor(private mapSvc: MapService) {
    this.mapSvc.getPolygons().valueChanges().subscribe((polygons: any[]) => {
      let polys = polygons.map(polygon => {
        console.log(polygon);
        if (polygon.id && polygon.paths && polygon.paths.length > 2) {
          polygon.paths.push(polygon.paths[0]);
          let newPaths = polygon.paths.map(path => {
            return [path.lat, path.lng];
          });
          let newGeo = new GeoJson('Polygon', [newPaths], { dbId: polygon.id });
          return {
            layer: {
              id: polygon.id,
      ...
      polys = polys.filter(poly => {
        return poly != null && poly != undefined;
      });
      console.log('polys:', polys);
      let featureColl = new FeatureCollection(polys);
      for (let poly of polys) {
        this.draw.add(poly);
      }
      // console.log('feature collection:', featureColl);
      // let featureIds = this.draw.add(featureColl);
  ...
  buildMap() {
    ...
    this.map.on('load', event => {
      ...
      // let featureCollection = new FeatureCollection([newGeoJSON]);
      // let ids = this.draw.set({ featureCollection });
      ...
      // let featureIds = this.draw.add(newLayer.source);
      // console.log('featureIds', featureIds);
    });
    ...
    this.map.on('click', 'newId', e => {
      console.log('clicked:', e);
      let featureIds = this.draw.add(e.features[0]);
      console.log('featureIds:', featureIds);
    });
